	...
	<body class="bar">
		...
		<ul class = "nav">
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){//Para cuando recargue la pagina y le envia la data que ingreso el usuario para insert delete
				$query= "DELETE FROM book WHERE idbook=".$_POST['id'].';';
				if(mysqli_query($con,$query)){
					$query= "DELETE FROM bookauthors WHERE idbook=".$_POST['id'].';';
					if (!mysqli_query($con,$query)) {
						echo '<script>
							alert("Error");
					 	 </script>';
			...
			echo '</select>';
			echo '<div>'.'<input id="menu_bts_1" type="submit" name="save_button" value="Delete Book" >'."</div>";
			echo '</form>';
		?>
	</body>
</html>
